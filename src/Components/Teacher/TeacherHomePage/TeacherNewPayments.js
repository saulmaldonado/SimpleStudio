        ...
        <div className='newly-submitted-payments-container' >
            ...
            <div className='newly-submitted-payments' >
                {this.props.teacherInfo.map((ele, i ) => {
                    return <PaymentBlock key={i} paymentId={ele.payment_id}paymentDueDate={ele.payment_duedate} paymentDate={ele.payment_date} paymentAmount={ele.payment_amount} studentName={`${ele.student_first_name} ${ele.student_last_name}`}/>
                })}
            </div>
        </div>
        )
